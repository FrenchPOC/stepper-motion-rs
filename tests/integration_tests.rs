...
use stepper_motion::config::{LimitPolicy, MechanicalConstraints, SoftLimits, SystemConfig};
...
fn t018_parse_trajectory_config() {
    ...
    let home = config
        .trajectory("home")
        .expect("Home trajectory should exist");
...
fn t021_contract_valid_config_produces_struct() {
    ...
    assert!(
        config.motor("pan").is_some(),
        "Declared motor MUST be accessible"
    );

    // Contract assertion 3: All declared trajectories are accessible
    assert!(
        config.trajectory("home").is_some(),
        "Declared trajectory MUST be accessible"
    );
    assert!(
        config.trajectory("asymmetric").is_some(),
        "Declared trajectory MUST be accessible"
    );

    // Contract assertion 4: Non-existent names return None
    assert!(
        config.motor("nonexistent").is_none(),
        "Non-existent motor MUST return None"
    );
    assert!(
        config.trajectory("nonexistent").is_none(),
        "Non-existent trajectory MUST return None"
    );
}
...
fn t051_complete_execution_workflow() {
    ...
    for (name, trajectory) in registry.iter() {
        ...
        assert!(
            motor.is_some(),
            "Trajectory '{}' references invalid motor '{}'",
            name,
            trajectory.motor
        );
